...
import { getLineage } from "@/lib/intelligence";
...
export async function GET(
  ...
  try {
    ...
    const lineage = getLineage(objects, edges, id);

    logResponse(requestId, 200, Date.now() - start);
    return successResponse(lineage);
  } catch (error: any) {
    logError(requestId, error);
    return errorResponse(new IntelligenceError("Lineage retrieval failed", { original: error.message }), requestId);
  }
}
